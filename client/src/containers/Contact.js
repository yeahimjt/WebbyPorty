  ...
  return (
    <div className={theme==="light"?"pb-32":"pb-32 bg-[color:var(--black)]"}>
      ...
      <form className="flex flex-col justify-center items-center max-w-[914px] gap-6  mx-auto p-8">
        <input className={`${theme==="light"? "w-full resize-y px-4 py-4 rounded-lg bg-[color:#E8E8E8] focus:scale-105 transition-all" : "w-full resize-y px-4 py-4 rounded-lg bg-[color:#777777] text-white focus:scale-105 transition-all placeholder-white"} ${myElementIsVisible4 ? 'animate-fade-right animate-once animation-delay-100 animate-duration-1000  animate-ease-in-out animate-normal' : ''}`} type="text" placeholder="Full Name" value={name || ''} onChange={(e)=> setName(e.target.value)}/>
        <input  className={`${theme==="light"? " w-full resize-y px-4 py-4 rounded-lg bg-[color:#E8E8E8] focus:scale-105 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500 transition-all" : "w-full resize-y px-4 py-4 rounded-lg bg-[color:#777777] text-white focus:scale-105 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-red-300 focus:invalid:border-pink-500 focus:invalid:ring-pink-500 transition-all placeholder-white"} ${myElementIsVisible4 ? 'animate-fade-left animation-delay-200 animate-once animate-duration-1000 animate-ease-in-out animate-normal' : ''}`} type="email" value={email || ''} placeholder="Email Address" onChange={(e)=> setEmail(e.target.value)}/>
        <textarea className={`${theme==="light"? " w-full resize-y px-4 py-4 rounded-lg bg-[color:#E8E8E8] focus:scale-105 transition-all" : "w-full resize-y px-4 py-4 rounded-lg bg-[color:#777777] text-white focus:scale-105 transition-all placeholder-white"} ${myElementIsVisible4 ? 'animate-fade-right animate-once animate-duration-1000 animation-delay-300 animate-ease-in-out animate-normal' : ''}`} type="text" placeholder="Your Message" value={body || ''} onChange={(e)=> setBody(e.target.value)}/>
        <button className={`${"bg-test w-full py-4 rounded-lg mt-4 text-white hover:scale-105  transition-all" } ${myElementIsVisible4 ? 'animate-fade-left animate-once animate-duration-1000 animation-delay-400 animate-ease-in-out animate-normal ' : ''}`} onClick={(e)=> {handleSubmit(e)}}>Start a Conversation</button>
      </form>
    </div>
  )
}
