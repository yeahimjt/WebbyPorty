  ...
  return (
    <div className={theme==="light"?"pb-32":"pb-32 bg-[color:var(--black)]"}>
      ...
      <div ref={myRef4} className="flex gap-6 mobile:flex-row flex-col justify-center py-16 flex-wrap">
        <a href="https://linkedin.com/in/jonathanandrewtrevino" target="_blank" rel="noreferrer" className="flex gap-2 w-[80%] mobile:w-max mx-auto mobile:m-0 items-center justify-center bg-[color:#0077B5] rounded-lg px-8 py-2 text-white hover:scale-105 hover:shadow-lg hover:cursor-pointer font-paragraph"><SiLinkedin size={32}/><p className="text-base">LinkedIn</p></a>
        <a href="mailto:yeahimjt@gmail.com" className={theme==="light"?"flex gap-2 items-center justify-center border-2 w-[80%]  mobile:w-max mx-auto mobile:m-0 border-[color:var(--light-blue)] px-8 py-2 rounded-lg hover:scale-105 hover:shadow-lg hover:cursor-pointer font-paragraph":"flex w-[80%]  mobile:w-max mx-auto mobile:m-0 gap-2 items-center justify-center border-2 border-[color:var(--light-blue)] px-8 py-2 text-white font-paragraph rounded-lg hover:scale-105 hover:shadow-lg hover:cursor-pointer"}><BsMailbox2 size={32} /><p className="text-base">yeahimjt@gmail.com</p></a>
      </div>
      <form className="flex flex-col justify-center items-center max-w-[914px] gap-6  mx-auto p-8">
        <input className={`${theme==="light"? "w-full mobile:w-full resize-y px-4 py-4 rounded-lg bg-[color:#E8E8E8] focus:scale-105 transition-all" : "w-full resize-y px-4 py-4 rounded-lg bg-[color:#777777] text-white focus:scale-105 transition-all placeholder-white"} ${myElementIsVisible4 ? 'animate-fade-right animate-once animate-duration-1000 animate-ease-in-out animate-normal' : ''}`} type="text" placeholder="Full Name" value={name || ''} onChange={(e)=> setName(e.target.value)}/>
        <input  className={`${theme==="light"? "w-full mobile:w-full resize-y px-4 py-4 rounded-lg bg-[color:#E8E8E8] focus:scale-105 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500 transition-all" : "w-full resize-y px-4 py-4 rounded-lg bg-[color:#777777] text-white focus:scale-105 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-red-300 focus:invalid:border-pink-500 focus:invalid:ring-pink-500 transition-all placeholder-white"} ${myElementIsVisible4 ? 'animate-fade-left animate-once animate-duration-1000 animate-ease-in-out animate-normal' : ''}`} type="email" value={email || ''} placeholder="Email Address" onChange={(e)=> setEmail(e.target.value)}/>
        <textarea className={`${theme==="light"? "w-full mobile:w-full resize-y px-4 py-4 rounded-lg bg-[color:#E8E8E8] focus:scale-105 transition-all" : "w-full resize-y px-4 py-4 rounded-lg bg-[color:#777777] text-white focus:scale-105 transition-all placeholder-white"} ${myElementIsVisible4 ? 'animate-fade-right animate-once animate-duration-1000 animate-ease-in-out animate-normal' : ''}`} type="text" placeholder="Your Message" value={body || ''} onChange={(e)=> setBody(e.target.value)}/>
        <button className={`${"bg-test w-full py-4 rounded-lg mt-4 text-white hover:scale-105  transition-all" } ${myElementIsVisible4 ? 'animate-fade-left animate-once animate-duration-1000 animate-ease-in-out animate-normal' : ''}`} onClick={(e)=> {handleSubmit(e)}}>Start a Conversation</button>
      </form>
    </div>
  )
}
